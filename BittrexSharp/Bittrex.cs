...
namespace BittrexSharp
{
    public class Bittrex
    {
        ...
        /// <summary>
        ///  Sigil Tech method to set api keys within bittrex.cs
        /// </summary>
        public void SetApiKeys(string Api, string ApiSecret)
        {
            this.apiKey = Api;
            this.apiSecret = ApiSecret;
            this.apiSecretBytes = encoding.GetBytes(apiSecret);
        ...
        public virtual async Task<MarketSummary> GetMarketSummary(string marketName)
        {
            ...
            JToken jsonResponse = await Request(HttpMethod.Get, uri, parameters, false);

            string marketSummary = jsonResponse.ToString();

            marketSummary = marketSummary.Substring(1, marketSummary.Length - 2);
            JToken AlteredjsonResponse = JToken.Parse(marketSummary);

            var Alteredmarketsummary = AlteredjsonResponse.ToObject<MarketSummary>();

            return Alteredmarketsummary;
